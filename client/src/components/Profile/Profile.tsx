...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchOwnedPosts = async () => {
      try {
        const response = await fetch(
          `http://localhost:3001/api/post/owned/${loggedIn}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${authToken}`,
        ...
        if (response.ok) {
          ...
          setOwnedPosts(null);
          console.error(
            `Error fetching owned post data: ${response.statusText}`
          );
        }
      } catch (error) {
  ...
  const filteredPosts = ownedPosts
    ? ownedPosts.filter((post) =>
        post.pet_name.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : [];
                            ...
                            <div className="card ">
                              {post.photo.length > 0 ? (
                                <img
                                  ...
                                  src={`http://localhost:3001/uploads/${post.photo[0].photo}`}
                                  alt={`Pirmoji nuotrauka ${post.pet_name}`}
                                  style={{
                                    width: "100%",
                                    height: "300px",
                                    objectFit: "cover",
                              ...
                              <div className="card-body">
                                <h5 className="card-title">
                                  <strong>Vardas:</strong> {post.pet_name}
                                </h5>
                                <p className="card-text">
                                  <strong>Aprašymas:</strong> {post.description}
                                </p>
                                <p className="card-text">
                                  <strong>Įkėlimo Data:</strong>{" "}
